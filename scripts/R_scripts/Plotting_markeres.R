...
fake_cpm_seurat <- readRDS(file = "/Volumes/home/Data analysis/MDS-Cibersortx/MDS_R/Query/Blast_analysis/marker_plot/fake_seurat.rds")

fake_cpm_seurat <- ScaleData(fake_cpm_seurat)

fake_cpm_seurat$Condition<- c("Blast", "Blast", "Blast", "Blast", "WT", "WT", "WT", "WT",
                              "WT", "WT")
fake_cpm_seurat$Sample.name<- targets$Sample.name
...
Vector <- c("Car2", "Eif5a", "Npm1","Ddx5", "TK1","MYBL2", "Eno1","TCF19")
...
Vector_mouse <- c("Cd34", "Crhbp", "Tal1", "Gata1", "Spi1", "Tk1", "Rrm2", 
                  "Itga2b", "Klf1", "Csf1", "Tfr2", "Cnrip1", "Kit", "Myc", 
                  "Csf3r", "Csf1r", "Tfrc", "Lmna", "Nfia", "Mybl2", "Batf3", "Tcf19",
                  "Sox7","Cd22", "Ms4a1", "Bcl2" ,"Flt3", "Flt3l", "Il2" ,"Il7", "Il7r",
                  "Cd3d","Cd3g", "Cd4", "Cd8a")


dittoDotPlot(fake_cpm_seurat, Vector,group.by  = "Sample.name" , min.color = "#1984c5", max.color = "#c23728", size=3, y.reorder = c(1:10))+
  coord_flip()+theme(axis.text.y = element_text(face= "bold", size= 10))+theme(axis.text.x = element_text(face= "bold", size= 7))

dittoHeatmap(fake_cpm_seurat, assay = "RNA",slot = "count",  Vector_mouse, group.by  = "Sample.name" ,show_colnames = T, 
             main = "Heatmap_marker_genes_relative_expression", min.color = "#1984c5", max.color = "#c23728", size=3, legend.color.title = "relative_expression" )
...
dittoBoxPlot(fake_cpm_seurat, c( "Cd3d","Cd3g", "Cd4", "Cd8a" ), group.by = "Condition", boxplot.lineweight = 0.1,, main = "Lymphoid_T")+
  theme(axis.text.x = element_text(face= "bold", size= 7))
